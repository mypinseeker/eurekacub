...
| 项 | 值 |
|---|---|
| **状态** | ✅ GATE-1 已批准 → ✅ GATE-2 已批准（2026-09-10，D-1/D-2/D-3 均选 A）→ ✅ **GATE-3 开发与 QA 完成**（FR-1 + FR-2，见迭代计划 §5）→ ✅ **已部署** `https://eurekacub.vercel.app`（2026-09-10，Vercel GitHub 集成自动部署 `a83fc61`）→ ⏳ GATE-4 待 A7 孩子真机验收 |
| **提出日期** | 2026-09-05 |
| **最后更新** | 2026-09-10 —— FR-1（分数 L4「一样多」）+ FR-2（对称 L4 平移、L5 旋转）已实现并通过门禁：单测 562 → **661**，E2E 79 → **89**。**已上线** `https://eurekacub.vercel.app`：线上 JS 与 `a83fc61` 的本地构建逐字节相同（见 A.9 末尾） |
...
- ~~部署到 Vercel~~ → **已完成**（2026-09-10）。用户新建了 Vercel 项目 `pinseekers-projects/eurekacub`，
  并接上了 GitHub 集成：`vercel[bot]` 在 19:22:51Z 自动把 `a83fc61` 部署到 Production，以后合并到 `main` 就会自动上线。核对结果：
  - `https://eurekacub.vercel.app` 返回 200；
  - 线上 `index-CmFAJ0Rb.js` 与本地构建**逐字节相同**；
  - 新关卡的特征字符串（一样多 / 平移魔法 / 旋转风车 / `equivalence-done` / `aha-popup`）都在；
  - 对照组：`@vite/client` 0 次；
  - 深层链接 `/module/m2/play/L4` 返回 200。
- ~~建议接上 Vercel 的 GitHub 集成~~ → 已接上
- **上线后的手机检查发现：对称关卡的画布都锁死在 150 像素高**（2026-09-10，在线上站实测）。
  - 实测：手机 390×844 和桌面 1280×720 下，镜像 L1、平移 L4、旋转 L5 的画布**都是 150 像素高**，正好是 canvas 元素的默认高度；
  - 原因：外层容器用了 `h-full`，但父容器没有明确高度；`CanvasBase` 量到的就是 canvas 默认的 150 像素，又把画布设成这个值，互相锁死；
  - 影响：平移和旋转在「边长取宽高较小值」的正方形里画，所以**不管屏幕多大，可画区域都只有 150×150 像素**；手机上旋转 L5 的叶片还压到了说明文字；
  - **已修新关卡**（`TransformCanvas` 高度改为 `min(65vh, 560px)`），并加了回归 E2E（可画正方形边长 ≥ 300）。反向验证：撤掉修复后量到 150，测试变红；
  - ⚠️ **镜像 L1–L3 有同样的问题，而且以前就有，这次没改**。迭代计划要求这三关一个像素都不变，要不要改请用户决定；同样的一行改法就能修
...
- [ ] 确认 A7 真机验收安排
- [x] 完成 Vercel 部署并核对线上版本（2026-09-10，线上 JS 与本地构建逐字节相同，见 A.10）
